...
@Component({
  selector: 'app-students-communitydetails-reg',
  templateUrl: './students-communitydetails-reg.component.html',
  styleUrls: ['./students-communitydetails-reg.component.css']
})
export class StudentsCommunitydetailsRegComponent implements OnInit {
  cols: any[] = [];
  ...
  getData() {
    this.emis_school_id = this.userSessionService.schoolId();
    this.registersService.getStudentsCommunityDetails(this.emis_school_id).subscribe((res) => {
      if (res) {
        this.result = this.data = res.result.school_community;
