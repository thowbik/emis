...
@Component({
  selector: 'app-students-validaadhar-reg',
  templateUrl: './students-validaadhar-reg.component.html',
  styleUrls: ['./students-validaadhar-reg.component.css']
})
export class StudentsValidaadharRegComponent implements OnInit {
  cols: any[] = [];
  ...
  getData() {
    this.emis_school_id = this.userSessionService.schoolId();
    this.registersService.getStudentsAadharDetails(this.emis_school_id).subscribe((res) => {
      if (res) {
