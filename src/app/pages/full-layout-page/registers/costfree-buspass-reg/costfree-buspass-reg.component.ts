...
@Component({
  selector: 'app-costfree-buspass-reg',
  templateUrl: './costfree-buspass-reg.component.html',
  styleUrls: ['./costfree-buspass-reg.component.css']
})
export class CostfreeBuspassRegComponent implements OnInit {
  cols: any[] = [];
  ...
  getData() {
    this.emis_school_id = this.userSessionService.schoolId();
    this.registersService.getCostFreeBuspassDetails(this.emis_school_id).subscribe((res) => {
      if (res) {
        this.data = res.result.buspass_distribute_details;
      }
    });
  }
  ...
  import("xlsx").then(xlsx => {
   const worksheet = xlsx.utils.json_to_sheet(this.getbuspass());
   const workbook = { Sheets: { 'Costfree-Buspass List' : worksheet }, SheetNames: ['Costfree-Buspass List'] };
   const excelBuffer: any = xlsx.write(workbook, { bookType: 'xlsx', type: 'array' });
   this.saveAsExcelFile(excelBuffer, this.emis_school_id +' (costfree-Busfree list) ');
  });
  }
