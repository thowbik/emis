      ...
      getclasslist(data,refresh)
  {
   ...
   getTrstseScholarshipData(data)
    ...
    getStudentSportsParticipation(data, refresh){
     ...
     getCostFreeBicycleDetails(schoolId , distrctID , BlockID){
      debugger
      return this.dataService.getDataSample('/scheme/bicycle?schId=' + schoolId + '&dstId=' + distrctID + '&blockId=' + BlockID , true )
    }

    
    getCompetitionRegisterData(schoolId)
    {
     return this.dataService.getDataSample('/contest/participantregister?schId='+schoolId,true);
    }
