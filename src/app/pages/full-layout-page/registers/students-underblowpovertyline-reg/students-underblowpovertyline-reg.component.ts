...
@Component({
  selector: 'app-students-underblowpovertyline-reg',
  templateUrl: './students-underblowpovertyline-reg.component.html',
  styleUrls: ['./students-underblowpovertyline-reg.component.css']
})
export class StudentsUnderblowpovertylineRegComponent implements OnInit {
  cols: any[] = [];
  ...
  getData() {
    this.emis_school_id = this.userSessionService.schoolId();
    this.registersService.getStudentsUnderbelowPovertyline(this.emis_school_id).subscribe((res) => {
      if (res) {
