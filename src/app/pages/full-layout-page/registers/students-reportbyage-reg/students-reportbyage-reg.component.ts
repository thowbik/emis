...
@Component({
  selector: 'app-students-reportbyage-reg',
  templateUrl: './students-reportbyage-reg.component.html',
  styleUrls: ['./students-reportbyage-reg.component.css']
})
export class StudentsReportbyageRegComponent implements OnInit {
  cols: any[] = [];
  ...
  getData() {
    this.emis_school_id = this.userSessionService.schoolId();
    this.registersService.getStudentsReportbyAge(this.emis_school_id).subscribe((res) => {
      if (res) {
        this.result =  res.result.student_age;
