...
@Component({
  selector: 'app-costfree-socks-reg',
  templateUrl: './costfree-socks-reg.component.html',
  styleUrls: ['./costfree-socks-reg.component.css']
})
export class CostfreeSocksRegComponent implements OnInit {
  cols: any[] = [];
  ...
  getData() {
    this.emis_school_id = this.userSessionService.schoolId();
    this.registersService.getCostFreeSocksDetails(this.emis_school_id).subscribe((res) => {
      if (res) {
        this.data = res.result.socks_distribute_details;
      }
    });
  }
  getcostfreesocks(){
    ...
    this.data.map(item => {
      return {
        ...
        'Distribution Date': item.distribution_date,
         ...
         let socks = [];
         for(let costfreesocks of this.exportExcelData) {
          socks.push(costfreesocks);
         }
         return socks;
   }
   ...
   exportExcel() {
    import("xlsx").then(xlsx => {
        const worksheet = xlsx.utils.json_to_sheet(this.getcostfreesocks());
        const workbook = { Sheets: { 'Cost Free Socks' : worksheet }, SheetNames: ['Cost Free Socks'] };
        const excelBuffer: any = xlsx.write(workbook, { bookType: 'xlsx', type: 'array' });
        this.saveAsExcelFile(excelBuffer, this.emis_school_id +' (Cost Free Socks ) ');
    });
  }
