...
export class StaffApprovalComponent implements OnInit {
  ...
  staff: Array<{'field':string,'header':string,'widthstyle':string}> =
   ...
   getStaffListApproval() {
    debugger;
    this.ceoservice.getStaffListApproval().subscribe((res) => {
      if (res) {
  ...
  onRowEdit_Save(Staffdata: StaffListApproval) {
                ...
                this.ceoservice.StudentDcApproval(records).subscribe((res) => {
                  debugger;
...
if(Staffdata.remrks) {
                  ...
                  this.ceoservice.StudentDcReject(records).subscribe((res) => {
                    debugger;
                  if (res.dataStatus == true) {
...
//       const worksheet = xlsx.utils.json_to_sheet(this.getStaffListApprovalExcel());
