...
/** TO Gathering Noonmeal Details In Class Wise  
 *  Last Modified Date : 04/12/2019 (venba/ps)
 * */
@Component({
  selector: 'app-noonmeal',
  templateUrl: './noonmeal.component.html',
  styleUrls: ['./noonmeal.component.css']
})

export class NoonmealComponent implements OnInit,OnDestroy {
    ...
    dataHeader: Array<{'field':string,'header':string,'widthstyle':string}> = [ { field:'unique_id_no',header:'EMIS ID' ,widthstyle: ''},
                                                                     { field: 'name', header: 'Student Name' ,widthstyle: ''},
                                                                     { field: 'class_studying_id', header: 'Class - Sec',widthstyle: ''},
                                                                     { field: 'is_opted', header: 'Opted for Noon Meal' ,widthstyle: '15em'},
                                                                     { field: 'indent', header: 'Updated Date',widthstyle: '' }
                                                                   ];
    ...
    toGetSectionDetails(school_id){
      ...
      toGetNoonmealDetails(Arr){
        this.studentService.getnoonmealList(Arr).subscribe(
          list=>{
            this.noonmealListArr = list['result']; 
            this.noonmealListArr.map(x=>x['status'] = (x['is_opted'] == '1') ? true : false );
    ...
    /** Datatable Save */
    onSubmit(Arr) : void{
      let _cls = this.filterForm.get('class_id').value;
      let _sec = this.filterForm.get('section').value;
      ...
      if(Arr.length>0){
        Arr.forEach((val, inx) => {
                      this.finalSaveArr.push({
                            ...
                            "distribution_date":this.pipe.transform(new Date(),'yyyy/MM/dd'), 
                            "is_opted": val['is_opted'],
                            "class":val['class_studying_id'],
                            "section" :val['class_section'],
                            "finalsub":val['noonmeal_primaryid'] == null ? 1 : 2,
                            "id":val['noonmeal_primaryid']
                          });
        });
                
        if(this.finalSaveArr.length>0){   
          this.studentService.saveAndUpdateSchemesdetails('/api/saveSchemes',{"records": {"data":this.finalSaveArr,"constant":"NOONMEAL"}}).subscribe(
            result=>{
              console.info(result['message']);
              this.alertService.success(result['message']);
              this.fetchingDetails(_cls,_sec);
          }); 
        }
        this.finalSaveArr = []; 
      }
    ...
    childToggle(e,data) {
      ...
      // console.log(data);
      data.status = e;
      data.is_opted = (data.status == true) ? '1' : '0';
      this.parentsCheckbox = this.isAllSelected();
    }
